...
public class ScannerFragment extends DialogFragment{
    ...
    public Dialog onCreateDialog(final Bundle savedInstanceState) {
        ...
        ForegroundColorSpan span = new ForegroundColorSpan(getResources().getColor(android.R.color.holo_red_dark));
    ...
    private void startScan() {
        ...
        if (ContextCompat.checkSelfPermission(getContext(), Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            ...
            if (ActivityCompat.shouldShowRequestPermissionRationale(getActivity(), Manifest.permission.ACCESS_COARSE_LOCATION)) {
                Toast.makeText(getActivity(),"Android 6.0 we need to obtain either Manifest.permission.ACCESS_COARSE_LOCATION ",Toast.LENGTH_LONG).show();
                return;
            }
        ...
        switch (requestCode) {
            case REQUEST_PERMISSION_REQ_CODE: {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(getActivity(), "no_required_permission", Toast.LENGTH_SHORT).show();
                }
                break;
            }
        }
